...
export async function PATCH(req, { params }) {
  try {
    ...
    if (data.add === true) {
      const add = await SwipedRight.findOneAndUpdate(
        { email },
        { $push: { petId: data.petId } },
        { returnDocument: "after", upsert: true }
      );
      if (!add) {
        return new NextResponse("Failed to add pet", { status: 500 });
      }
      return new NextResponse(JSON.stringify(add), {
        status: 200,
      ...
      const remove = await SwipedRight.findOneAndUpdate(
        { email },
        { $pull: { petId: data.petId } },
        { returnDocument: "after", upsert: true }
      );
      if (!remove) {
...
export async function GET(req, { params }) {
  try {
    ...
    if (!myWishlist) {
      return new NextResponse("No wishlist found", { status: 200 });
    }
    return new NextResponse(JSON.stringify(myWishlist), {
      status: 200,
      headers: { "content-type": "application/json" },
    });
  } catch (error) {}
